    ...
    try:
        ...
        if client:
            ...
            try:
                ...
                data = worksheet.get_all_records()
                data = [row for row in data if row.get("ID") != "" and row.get("ID") is not None and str(row.get("DÖNEM")).strip() != "Eylül 2026"]
                df = pd.DataFrame(data)
                if df.empty:
                    return []
            except:
...
def update_gecmis_google_sheet(liste_veri):
    try:
        ...
        filtered_liste = [item for item in liste_veri if str(item.get("DÖNEM")).strip() != "Eylül 2026"]
        df_to_write = pd.DataFrame(filtered_liste)
        for col in GECMIS_SUTUNLAR:
        ...
        if not df_to_write.empty:
            worksheet.update([df_to_write.columns.values.tolist()] + df_to_write.values.tolist())
        st.session_state["gecmis_ozetler"] = filtered_liste
        return True
    except Exception as e:
...
if "chat_messages" not in st.session_state:
    st.session_state["chat_messages"] = [
        {"zaman": "19.09.2026 10:30", "gonderen": "Mehmet ALAŞAR", "mesaj": "Ömer Bey, güncel raporları kontrol edebilir misiniz?", "dosya_linki": ""}
    ]
...
with tab1:
    if user["role"] == "admin":
        ...
        # --- TOPLU EXCEL/DOSYA YÜKLEME VE NOKTA ATIŞI AKTARIM ÖZELLİĞİ ---
        with st.expander("📤 Excel Dosyasından Toplu Veri Yükle (Nokta Atışı Aktarım)", expanded=False):
            st.markdown("Lütfen Excel dosyanızı yükleyin. Sistem; **Referans**, **Seri Sorumlusu**, **Uygulanan İşlem(Açıklama)**, **Adet**, **PH**, **Talep Edilen Saat** sütunlarını eşleştirerek Google Sheets aktif sayfasına işleyecektir.")
            uploaded_excel = st.file_uploader("Excel Dosyası Seç (.xlsx, .xls, .csv)", type=["xlsx", "xls", "csv"], key="excel_uploader_tab1")
            
            if uploaded_excel is not None:
                try:
                    if uploaded_excel.name.endswith('.csv'):
                        upl_df = pd.read_csv(uploaded_excel)
                    else:
                        upl_df = pd.read_excel(uploaded_excel)
                    
                    st.write("Yüklenen Dosya Önizlemesi:", upl_df.head(3))
                    
                    if st.button("🚀 Dosyadaki Verileri Google Sheets'e Aktar", type="primary", key="btn_excel_aktar"):
                        mevcut_df = load_data()
                        ...
                        for idx, row in upl_df.iterrows():
                            ...
                            yeni_satirlar.append({
                                ...
                                "Toplam Tutar (TL)": round(talep_saat_val * 500.0, 2),
                                "Son Durum": "Mail Atıldı",
                        ...
                        yeni_eklenen_df = pd.DataFrame(yeni_satirlar)
                        birlestirilmis_df = pd.concat([mevcut_df, yeni_eklenen_df], ignore_index=True)
                        if update_google_sheet(birlestirilmis_df):
                            st.success(f"Başarıyla {len(yeni_satirlar)} adet kayıt Excel dosyasından Google Sheets'e aktarıldı!")
                            st.rerun()
                except Exception as ex:
                    st.error(f"Dosya işleme hatası: {ex}")
