...
st.set_page_config(
    page_title="Müşteri Kayıp Zaman Takip Sistemi", layout="wide", page_icon="⏱️"
)
...
SUTUNLAR = [
    ...
    "Hata Oranı (%)",
    "P/H",
    "Hesaplanan Zaman (Saat)",
    "Kayıp Zaman (Saat)",
    "Saatlik İşçilik Ücreti (TL)",
    "Toplam Tutar (TL)",
    "Son Durum",
    "İrsaliye Görseli Linki",
    "Etiket Görseli Linki",
    "Hata Görseli Linki",
    "Onay Belgesi Linki",
]
...
def fetch_main_data_from_sheet():
  try:
    ...
    for num_col in [
      ...
      if num_col in df.columns:
        ...
        numeric_vals = (
            ...
            / (100.0 if "%" in num_col else 1.0)
        )
        df[num_col] = numeric_vals.round(2)

    df["Gelen Parti Miktarı"] = (
        pd.to_numeric(df["Gelen Parti Miktarı"], errors="coerce")
        .fillna(0)
        .astype(int)
    )
    ...
    if "Dönem (Ay/Yıl)" in df.columns:
      ...
      df["Dönem (Ay/Yıl)"] = df["Dönem (Ay/Yıl)"].replace(
          ["nan", "None", ""], "Eylül 2026"
      )
  ...
  try:
    ...
    for col in [
      ...
      if col in df_to_write.columns:
        num_series = (
            pd.to_numeric(df_to_write[col], errors="coerce").fillna(0.0).round(2)
        )
        df_to_write[col] = num_series.apply(
  ...
  try:
    ...
    if client:
      ...
      try:
        ...
        data = [
            row
            for row in data
            if row.get("ID") != "" and row.get("ID") is not None
        ]
        df = pd.DataFrame(data)
        if df.empty:
          return []
    ...
    for col in GECMIS_SUTUNLAR:
      if col not in df.columns:
        df[col] = (
            "0"
            if "SAAT" in col or "TUTAR" in col or "ÜCRET" in col
            else ""
        )
      else:
  ...
  try:
    ...
    try:
      ...
      worksheet = spreadsheet.add_worksheet(
          title="gecmisdonem", rows="100", cols="20"
      )
...
def update_odeme_google_sheet(liste_veri):
  try:
    ...
    try:
      ...
      worksheet = spreadsheet.add_worksheet(
          title="odeme-tahsilat-takip", rows="100", cols="20"
      )
    ...
    if "GERÇEKLEŞEN GELEN ÖZEL TUTAR" in df_to_write.columns:
      num_series = (
          df_to_write["GERÇEKLEŞEN GELEN ÖZEL TUTAR"]
          .apply(parse_float_tr)
          .round(2)
      )
      df_to_write["GERÇEKLEŞEN GELEN ÖZEL TUTAR"] = num_series.apply(
          lambda x: f"{x:.2f}".replace(".", ",")
      )
...
if "chat_messages" not in st.session_state:
  st.session_state["chat_messages"] = [{
      "zaman": "19.09.2026 10:30",
      "gonderen": "Mehmet ALAŞAR",
      "mesaj": "Ömer Bey, eylül ayı raporunu kontrol edebilir misiniz?",
      "dosya_linki": "",
  }]
  ...
  with col_b:
    ...
    with st.form("login_form"):
      ...
      if login_btn:
        ...
        if (
            secilen_kullanici in users
            and users[secilen_kullanici]["password"] == password_input
        ):
...
if st.sidebar.button(
    "🔄 Verileri Google Sheets'ten Yenile", use_container_width=True
):
  st.cache_data.clear()
  st.session_state["main_df"] = fetch_main_data_from_sheet()
  st.session_state["gecmis_ozetler"] = fetch_gecmis_from_sheet()
...
with tab1:
  if user["role"] == "admin":
    ...
    with col_f1:
      secilen_donem_form = st.selectbox(
          "Dönem Seçin (Ay / Yıl) *",
          options=DONEM_LISTESI,
          index=8,
          key="f_donem",
      )
      musteri_adi = st.text_input("Müşteri Adı *", key="f_musteri")
      sorumlu_muhendis = st.text_input(
          "Müşteri Sorumlu Mühendis Adı", key="f_muhendis"
      )
      ...
      durus_nedeni = st.selectbox(
          "Duruş / Zaman Kaybı Nedeni *", DURUS_NEDENLERI, key="f_neden"
      )

    with col_f2:
      gelen_parti = st.number_input(
          "Gelen Parti / Stok Miktarı (Adet)",
          min_value=1,
          value=14000,
          step=10,
          key="f_parti",
      )
      hata_orani = st.number_input(
          "Hata Oranı (%)",
          min_value=0.0,
          max_value=100.0,
          value=100.0,
      ...
      ph = st.number_input(
          "P/H (Parça / Saat)", min_value=0.1, value=520.0, step=1.0, key="f_ph"
      )

      canli_islem_adet = gelen_parti * (hata_orani / 100.0)
      canli_hesaplanan_saat = (
          round(canli_islem_adet / ph, 2) if ph > 0 else 0.0
      )
      ...
      if "Manuel" in zaman_modu:
        ...
        st.success(
            f"⏱️ **Hesaplanan Zaman: {hesaplanan_zaman:.2f} Saat**  \n*(Detay:"
            f" {int(canli_islem_adet)} Adet Hatalı Parça / {ph} P/H)*"
        )

      st.markdown("---")
      kayip_zaman_saat = st.number_input(
          "Kayıp Zaman / Fiili Duruş (Saat) *",
          min_value=0.0,
          value=26.92,
          step=0.1,
          format="%.2f",
      ...
      st.info(
          f"💰 **Hesaplanan Toplam Tutar: {toplam_tutar_hesaplanan:,.2f} TL**"
          .replace(",", "X")
          .replace(".", ",")
          .replace("X", ".")
      )

      son_durum = st.selectbox(
          "Son Durum *",
          ["Mail Atıldı", "Onay Geldi", "Red Oldu", "Revize İstendi"],
          key="f_durum",
      )

    islem_aciklamasi = st.text_area(
        "İşlem Açıklaması",
        placeholder="Yapılan işlem, duruş gerekçesi ve detaylar...",
        key="f_aciklama",
    )

    c_link0, c_link1, c_link2, c_link3 = st.columns(4)
    with c_link0:
      irsaliye_gorseli_link = st.text_input(
          "📦 İrsaliye Görseli Linki", key="f_irsaliye_link"
      )
    with c_link1:
      etiket_gorseli_link = st.text_input(
          "🏷️ Etiket Görseli Linki", key="f_etiket"
      )
    with c_link2:
    ...
    if st.button(
        "💾 Kaydı Google Tabloya Kaydet", use_container_width=True, type="primary"
    ):
      if not musteri_adi or not referans_no:
        ...
        valid_ids = valid_ids[
            pd.to_numeric(valid_ids, errors="coerce").notnull()
        ]
        yeni_id = (
            int(valid_ids.max()) + 1 if not valid_ids.empty else 1
        )
        ...
        if update_google_sheet(guncel_df):
          st.success(
              f"ID #{yeni_id} ({secilen_donem_form} - {musteri_adi}) başarıyla"
    ...
    donem_secenekleri = [
        (
            f"{item.get('DÖNEM', '')} — Onaylanan Tutar:"
            f" {parse_float_tr(item.get('ONAYLANAN TUTAR', 0)):,.2f} TL"
        )
        .replace(",", "X")
        .replace(".", ",")
        .replace("X", ".")
    ...
    secilen_donem_str = st.selectbox(
        "Dönem ve Onaylanan Tutarlar *",
        options=donem_secenekleri,
        key="muh_secilen_donem_str",
    )

    secilen_donem_adi = secilen_donem_str.split(" — ")[0]
    secilen_item = next(
        (item for item in gecmis_liste if item.get("DÖNEM") == secilen_donem_adi),
        None,
    )
    varsayilan_onaylanan_tutar = (
        parse_float_tr(secilen_item.get("ONAYLANAN TUTAR", 0.0))
        if secilen_item
        else 0.0
    )

    farkli_tutar_var_mi = st.checkbox(
        "Gelen tutar onaylanan tutardan farklı mı?",
        key="muh_farkli_tutar_check",
    )

    with st.form("muhasebe_odeme_form", clear_on_submit=True):
      col_m1, col_m2 = st.columns(2)
      with col_m1:
        muh_musteri = st.text_input(
            "Müşteri Adı *", value="Legrand", key="muh_musteri_adi"
        )
        muh_durum_tipi = st.selectbox(
            "Ödeme Durumu",
            options=["Gelen Ödeme", "Bekleyen Ödeme"],
            key="muh_durum_tipi",
        )
        muh_para_birimi = st.selectbox(
            "Para Birimi",
            options=["TL (₺)", "USD ($)", "EUR (€)"],
            key="muh_pb",
        )
      ...
      with col_m2:
        if farkli_tutar_var_mi:
          muh_tutar = st.number_input(
              "Gerçekleşen / Gelen Özel Tutar *",
              min_value=0.0,
              value=float(varsayilan_onaylanan_tutar),
              step=100.0,
              format="%.2f",
              key="muh_ozel_tutar",
          )
        ...
        muh_kur = st.number_input(
            "TCMB Kur / Çevrim Çarpanı",
            min_value=0.0001,
            value=1.0 if "TL" in muh_para_birimi else 35.0,
            step=0.01,
            format="%.4f",
            key="muh_kur_val",
        )

      muh_aciklama = st.text_area(
          "Açıklama / Notlar", placeholder="Notlar...", key="muh_aciklama_val"
      )

      btn_odeme_kaydet = st.form_submit_button(
          "💾 Ödeme / Tahsilat Kaydet", use_container_width=True, type="primary"
      )
      ...
      if btn_odeme_kaydet:
        if not muh_musteri.strip() or muh_tutar < 0:
          ...
          yeni_odeme_id = (
              max(
                  [
                      parse_float_tr(item.get("ID", 0))
                      for item in mevcut_odemeler
                  ]
              )
              + 1
              if mevcut_odemeler
          ...
          yeni_kayit_dict = {
              ...
              "AÇIKLAMA NOTLAR": (
                  muh_aciklama.strip()
                  if muh_aciklama.strip()
                  else f"{secilen_donem_adi} dönemi tahsilatı."
              ),
          ...
          if update_odeme_google_sheet(mevcut_odemeler):
            st.success(
                "Ödeme kaydı sisteme ve Google Sheets'e başarıyla işlendi!"
            )
            st.rerun()
  else:
    ...
    if df.empty or df.dropna(how="all").empty:
      ...
      edited_df = st.data_editor(
          ...
          column_config={
              ...
              "Kayıp Zaman Türü": st.column_config.SelectboxColumn(
                  "Kayıp Zaman Türü",
                  options=KAYIP_ZAMAN_TURU_OPSIYONLARI,
                  required=True,
              ),
              "Dönem (Ay/Yıl)": st.column_config.SelectboxColumn(
                  "Dönem (Ay/Yıl)", options=DONEM_LISTESI, required=True
              ),
              "Hata Oranı (%)": st.column_config.NumberColumn(
                  "Hata Oranı (%)", min_value=0.0, max_value=100.0, format="%.2f"
              ),
              "Hesaplanan Zaman (Saat)": st.column_config.NumberColumn(
                  "Hesaplanan Zaman (Saat)", format="%.2f"
              ),
              "Kayıp Zaman (Saat)": st.column_config.NumberColumn(
                  "Kayıp Zaman (Saat)", format="%.2f"
              ),
              "Saatlik İşçilik Ücreti (TL)": st.column_config.NumberColumn(
                  "Saatlik Ücret (TL)", format="%.2f"
              ),
              "Toplam Tutar (TL)": st.column_config.NumberColumn(
                  "Toplam Tutar (TL)", format="%.2f"
              ),
              ...
              "Duruş Nedeni": st.column_config.SelectboxColumn(
                  "Duruş Nedeni", options=DURUS_NEDENLERI, required=True
              ),
          },
      ...
      with c_kaydet:
        if st.button(
            "🔄 Tablo Değişikliklerini Google Sheets'e Kaydet",
            use_container_width=True,
        ):
          clean_df = edited_df.drop(columns=["Seç"], errors="ignore")
          if update_google_sheet(clean_df):
            st.success(
                "Tablo değişiklikleri başarıyla Google Sheets'e kaydedildi!"
            )
            st.rerun()
      with c_secili_sil:
        if st.button(
            "🗑️ İşaretlenen Kayıtları Sil",
            use_container_width=True,
            type="primary",
        ):
          ...
          if secilenler.empty:
            st.warning(
                "Lütfen silmek istediğiniz satırın solundaki 'Seç' kutucuğunu"
                " işaretleyin!"
            )
          else:
            kalan_df = (
                edited_df[edited_df["Seç"] == False]
                .drop(columns=["Seç"], errors="ignore")
            )
            if update_google_sheet(kalan_df):
              st.success(
                  f"Seçilen {len(secilenler)} adet kayıt Google Sheets'ten"
                  " silindi!"
              )
              st.rerun()
      with c_indir:
      ...
      with col_id1:
        silinecek_id_input = st.number_input(
            "Silmek İstediğiniz Kaydın ID Numarası",
            min_value=1,
            step=1,
            key="tekli_sil_id_input",
        )
      with col_id2:
        st.markdown("<br>", unsafe_allow_html=True)
        if st.button(
            "❌ Bu ID'ye Sahip Kaydı Sil", type="primary", use_container_width=True
        ):
          mevcut_df_id = load_data()
          if (
              "ID" in mevcut_df_id.columns
              and silinecek_id_input in mevcut_df_id["ID"].values
          ):
            yeni_temiz_df = mevcut_df_id[
                mevcut_df_id["ID"] != silinecek_id_input
            ]
            if update_google_sheet(yeni_temiz_df):
              st.success(
                  f"ID #{silinecek_id_input} numaralı kayıt başarıyla silindi!"
              )
              st.rerun()
          else:
            st.error(
                f"Sistemde #{silinecek_id_input} ID numarasına ait kayıt"
                " bulunamadı."
            )
  else:
...
with tab3:
  ...
  for col in GECMIS_SUTUNLAR:
    if col not in manuel_df.columns:
      manuel_df[col] = (
          0.0 if "SAAT" in col or "TUTAR" in col or "ÜCRET" in col else ""
      )
  ...
  aktif_talep = (
      pd.to_numeric(df["Hesaplanan Zaman (Saat)"], errors="coerce").sum()
      if not df.empty and "Hesaplanan Zaman (Saat)" in df.columns
      else 0.0
  )
  aktif_onay = (
      pd.to_numeric(
          df.loc[df["Son Durum"] == "Onay Geldi", "Hesaplanan Zaman (Saat)"],
          errors="coerce",
      ).sum()
      if not df.empty and "Hesaplanan Zaman (Saat)" in df.columns
      else 0.0
  ...
  m_talep = (
      manuel_df["TALEP EDİLEN SAAT"].apply(parse_float_tr).sum()
      if not manuel_df.empty
      else 0.0
  )
  m_onay = (
      manuel_df["ONAYLANAN SAAT"].apply(parse_float_tr).sum()
      if not manuel_df.empty
      else 0.0
  )
  ...
  m_talep_tutar = (
      manuel_df["TALEP EDİLEN TUTAR"].apply(parse_float_tr).sum()
      if not manuel_df.empty
      else 0.0
  )
  m_onay_tutar = (
      manuel_df["ONAYLANAN TUTAR"].apply(parse_float_tr).sum()
      if not manuel_df.empty
      else 0.0
  )
  m_kesinti_tutar = (
      manuel_df["KESİNTİ TUTARI"].apply(parse_float_tr).sum()
      if not manuel_df.empty
      else 0.0
  )
  ...
  ft1.metric(
      "Geçmiş Toplam Talep Edilen Tutar",
      f"{m_talep_tutar:,.2f} TL"
      .replace(",", "X")
      .replace(".", ",")
      .replace("X", "."),
  )
  ft2.metric(
      "Geçmiş Toplam Onaylanan Tutar",
      f"{m_onay_tutar:,.2f} TL".replace(",", "X").replace(".", ",").replace("X", "."),
  )
  ft3.metric(
      "Geçmiş Toplam Kesinti Tutarı",
      f"{m_kesinti_tutar:,.2f} TL"
      .replace(",", "X")
      .replace(".", ",")
      .replace("X", "."),
  )
  ...
  if not odeme_analiz_df.empty:
    ...
    odeme_analiz_df["TL_Karsiligi"] = odeme_analiz_df.apply(
        hesapla_tl_karsilik, axis=1
    )

    gelen_toplam = odeme_analiz_df[
        odeme_analiz_df["ÖDEME DURUMU"] == "Gelen Ödeme"
    ]["TL_Karsiligi"].sum()
    bekleyen_toplam = odeme_analiz_df[
        odeme_analiz_df["ÖDEME DURUMU"] == "Bekleyen Ödeme"
    ]["TL_Karsiligi"].sum()
    ...
    st.dataframe(
        odeme_analiz_df.drop(columns=["TL_Karsiligi"], errors="ignore"),
        use_container_width=True,
    )
  else:
  ...
  if not df.empty and not df.dropna(how="all").empty:
    ...
    with cg1:
      g_mus = (
          df.groupby("Müşteri Adı")["Hesaplanan Zaman (Saat)"]
          .sum()
          .reset_index()
      )
      fig_m = px.bar(
          ...
          title="Müşteri Bazlı Toplam Zaman",
      )
      st.plotly_chart(fig_m, use_container_width=True)
    with cg2:
      g_ned = (
          df.groupby("Duruş Nedeni")["Hesaplanan Zaman (Saat)"]
          .sum()
          .reset_index()
      )
      fig_n = px.bar(
          ...
          x="Duruş Nedeni",
          y="Hesaplanan Zaman (Saat)",
          title="Duruş Nedenleri Dağılımı",
      )
    ...
    fig_trend = px.line(
        ...
        color="Müşteri Adı",
        markers=True,
        title="Dönemsel Zaman Trend Analizi",
    )
...
with tab4:
  ...
  for col in GECMIS_SUTUNLAR:
    if col not in gecmis_temp_df.columns:
      gecmis_temp_df[col] = (
          "0" if "SAAT" in col or "TUTAR" in col or "ÜCRET" in col else ""
      )
    else:
      gecmis_temp_df[col] = (
          gecmis_temp_df[col].astype(str).replace(["nan", "None"], "0")
      )
  ...
  if is_admin:
    gecmis_df_editable = st.data_editor(
        ...
        column_config={
            ...
            "TALEP EDİLEN SAAT": st.column_config.TextColumn(
                "TALEP EDİLEN SAAT"
            ),
            "ONAYLANAN SAAT": st.column_config.TextColumn("ONAYLANAN SAAT"),
            "SAATLİK ÜCRET": st.column_config.TextColumn("SAATLİK ÜCRET"),
            "TALEP EDİLEN TUTAR": st.column_config.TextColumn(
                "TALEP EDİLEN TUTAR", disabled=True
            ),
            "ONAYLANAN TUTAR": st.column_config.TextColumn(
                "ONAYLANAN TUTAR", disabled=True
            ),
            "KESİNTİ TUTARI": st.column_config.TextColumn("KESİNTİ TUTARI"),
            "KESİNTİ AÇIKLAMASI": st.column_config.TextColumn(
                "KESİNTİ AÇIKLAMASI"
            ),
    ...
    if st.button(
        "🔄 Saatleri Saatlik Ücretle Çarp & Sheets'e Kaydet",
        use_container_width=True,
        type="primary",
    ):
      updated_records = []
      ...
      if update_gecmis_google_sheet(updated_records):
        st.success(
            "Geçmiş dönem verileriniz başarıyla Google Sheets'e kaydedildi!"
        )
        st.rerun()
  else:
    st.info(
        "ℹ️ Geçmiş dönem verilerini yalnızca **Yönetici (Ömer OCAK)**"
        " düzenleyebilir veya silebilir. Bu sayfayı şu an salt okunur olarak"
        " görüntülemektesiniz."
    )
  ...
  with chat_container:
    if not st.session_state["chat_messages"]:
      ...
      for m in st.session_state["chat_messages"]:
        dosya_html = (
            f"<br>📎 <a href='{m['dosya_linki']}' target='_blank'>Dosyayı"
            " Aç</a>"
            if m.get("dosya_linki")
            else ""
        )
        st.markdown(
            f"**{m['gonderen']}** ({m['zaman']}): {m['mesaj']} {dosya_html}",
...
with tab6:
  st.subheader(
      "💰 Müşteri Ödemeleri & Tahsilat Takip Panosu (`odeme-tahsilat-takip`)"
  )
  ...
  if odeme_df.empty:
    ...
    if yetkili_mi:
      ...
      if st.button(
          "🔄 Ödeme Tablosu Değişikliklerini Google Sheets'e Kaydet",
          use_container_width=True,
      ):
  ...
  if yetkili_mi:
    ...
    with col_odm_1:
      if st.button(
        ...
        if secilen_odemeler.empty:
          st.warning(
              "Lütfen silmek istediğiniz ödeme satırının solundaki 'Seç'"
              " kutucuğunu işaretleyin!"
          )
        else:
          kalan_odemeler_df = (
              edited_odeme_df[edited_odeme_df["Seç"] == False]
              .drop(columns=["Seç"], errors="ignore")
          )
    ...
    with col_odm_2:
      if st.button(
          "⚠️ Tüm Ödeme Kayıtlarını Temizle",
          type="secondary",
          use_container_width=True,
      ):
    ...
    with col_oid1:
      silinecek_odeme_id = st.number_input(
          "Silmek İstediğiniz Ödeme ID Numarası",
          min_value=1,
          step=1,
          key="tekli_odeme_sil_id",
      )
    with col_oid2:
      st.markdown("<br>", unsafe_allow_html=True)
      if st.button(
          "❌ Bu ID'ye Sahip Ödemeyi Sil",
          type="primary",
          use_container_width=True,
      ):
        mevcut_odemeler = st.session_state["odeme_kayitlari"]
        yeni_odemeler = [
            item
            for item in mevcut_odemeler
            if parse_float_tr(item.get("ID")) != silinecek_odeme_id
        ]
        if len(yeni_odemeler) < len(mevcut_odemeler):
          if update_odeme_google_sheet(yeni_odemeler):
            ...
            st.success(
                f"ID #{silinecek_odeme_id} numaralı ödeme kaydı silindi!"
            )
            st.rerun()
        else:
          st.error(
              f"Sistemde #{silinecek_odeme_id} ID numarasına ait ödeme kaydı"
              " bulunamadı."
          )
  else:
